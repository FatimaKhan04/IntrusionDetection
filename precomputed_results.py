...
def compute_anomaly_scores(tensor_data):
    with torch.no_grad():
        recon = model(tensor_data)
        loss = ((tensor_data - recon) ** 2).mean(dim=1).numpy()
    return loss

val_scores = compute_anomaly_scores(X_val_tensor)
attack_scores = compute_anomaly_scores(X_attack_tensor)
